# auth.py — robuste au cold start Render + backup DB sur Supabase
# ---------------------------------------------------------------
# - Conserve toutes tes fonctions (users, sessions, submissions, import CSV…)
# - Restaure la DB depuis Supabase si absente (Render Free "cold start")
# - Sauvegarde la DB vers Supabase après chaque écriture
#   Variables Render nécessaires :
#     SUPABASE_URL
#     SUPABASE_SERVICE_ROLE_KEY   (secret)
#     SUPABASE_BUCKET             (ex: smartedittrack)
#     SUPABASE_DB_BACKUP_PATH     (ex: backups/smartedittrack.db)  ← nouveau
#     ADMIN_ID / ADMIN_PASSWORD   (bootstrap admin)
# ---------------------------------------------------------------
...
# --------- Supabase helpers (optionnels mais recommandés) ----------
_SUPA_OK = False
_DB_REMOTE_PATH = os.getenv("SUPABASE_DB_BACKUP_PATH", "backups/smartedittrack.db")
try:
    # Requiert supa.py (helpers Storage) dans le projet
...
def _restore_db_if_missing():
    """🧰 Si la DB locale est absente (cold start), tente de la restaurer depuis Supabase."""
...
def _backup_db():
    """🔁 Sauvegarde la DB locale vers Supabase (upsert)."""
...
# --------- DB ---------
def get_conn():
    # Restauration éventuelle avant ouverture (pour Render Free)
...
def change_password(conn, user_id, current_pwd, new_pwd):
    ...
    if not row or not _verify(row[0], current_pwd):
        return False
    set_password_for_user(conn, user_id, new_pwd)  # fait commit + backup
    return True
